...
public class SecurityConfig {
    /* springSecurity6 => createDelegation */
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        return http.csrf(csrf->csrf.disable())
                .authorizeHttpRequests(authorize -> authorize
                        .requestMatchers("/index","/","/js/**","/dist/**","/board/list", "/member/login","/member/login","/member/register","/upload/**","/comment/**").permitAll()
...
//        return http.build();
    ...
    UserDetailsService userDetailsService(){
        return new CustomUserService(); // Security 패키지에 클레스로 생성
    }
